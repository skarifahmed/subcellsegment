import matplotlib
matplotlib.use("Agg")  # Disable GUI backend (no display)
...
def surface_distances(gt, pred):
    ...
    return dt_pred[gt_surf], dt_gt[pred_surf]
...
def evaluate_model(model, X_test, y_test):
    ...
    inter = np.logical_and(y_test, preds_bin).sum()
    union = np.logical_or(y_test, preds_bin).sum()
    iou = inter / (union + 1e-6)

    return acc, f1, iou, preds

# ============================================================
# SAVE TEST RESULTS (RAW BINARY MASKS + METRICS)
# ============================================================

def save_test_results(X, y, preds_dict, out_dir, dataset_name, filenames):
    ...
    for i in range(n):
        ...
        unet_pred = preds_dict["UNet"][i,...,0]
        se_pred   = preds_dict["SEUNet"][i,...,0]
        cas_pred  = preds_dict["Cascaded"][i,...,0]

        unet_bin = (unet_pred > 0.5).astype(np.uint8)
        se_bin   = (se_pred > 0.5).astype(np.uint8)
        cas_bin  = (cas_pred > 0.5).astype(np.uint8)

        iou_unet = compute_iou(gt, unet_bin)
        iou_se   = compute_iou(gt, se_bin)
        iou_cas  = compute_iou(gt, cas_bin)

        hd95_unet = hd95(gt, unet_bin)
        hd95_se   = hd95(gt, se_bin)
        hd95_cas  = hd95(gt, cas_bin)

        asd_unet = asd(gt, unet_bin)
        asd_se   = asd(gt, se_bin)
        asd_cas  = asd(gt, cas_bin)
        ...
        rows.append({
            ...
            "SEUNet_HD95": hd95_se,
            "SEUNet_ASD": asd_se,
            "Cascaded_IoU": iou_cas,
            "Cascaded_HD95": hd95_cas,
            "Cascaded_ASD": asd_cas
        })

        safe_base = filenames[i].replace("/", "_").replace("\\", "_")
        unet_path = os.path.join(out_dir, f"{safe_base}_UNet.png")
        se_path   = os.path.join(out_dir, f"{safe_base}_SEUNet.png")
        cas_path  = os.path.join(out_dir, f"{safe_base}_Cascaded.png")

        cv2.imwrite(unet_path, (unet_bin * 255).astype(np.uint8))
        cv2.imwrite(se_path,   (se_bin   * 255).astype(np.uint8))
        cv2.imwrite(cas_path,  (cas_bin  * 255).astype(np.uint8))

    df = pd.DataFrame(rows)
    df.to_csv(f"results/{dataset_name}_per_image_metrics.csv", index=False)
    print(f"Saved per-image metrics to results/{dataset_name}_per_image_metrics.csv")
...
for train_name in DATASETS:
    ...
    acc_u, f1_u, iou_u, preds_u = evaluate_model(unet_model, X_test, y_test)
    acc_s, f1_s, iou_s, preds_s = evaluate_model(seunet_model, X_test, y_test)
    acc_c, f1_c, iou_c, preds_c = evaluate_model(casc_model, X_test, y_test)
    ...
    qual_dir = os.path.join("qualitative", f"{train_name}_test")
    save_test_results(X_test, y_test, preds_dict, qual_dir, train_name, names_test)

results_df = pd.DataFrame(results_rows)
results_df.to_csv("results/per_dataset_results_UNet_SEUNet_Cascaded.csv", index=False)
print("\n===== FINAL PER-DATASET RESULTS =====")
print(results_df)
